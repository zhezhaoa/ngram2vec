...
real *W, *gradsq, *cost;
long long num_lines, *lines_per_thread, vocab_size, words_size, contexts_size; //!@#$%^&*
char *vocab_file, *input_file, *save_W_file, *save_gradsq_file, *words_file, *contexts_file; //!@#$%^&*
...
void initialize_parameters() {
    ...
    //!@#$%^&*
    a = posix_memalign((void **)&W, 128, (words_size + contexts_size) * vector_size * sizeof(real)); // Might perform better than malloc
    if (W == NULL) {
        fprintf(stderr, "Error allocating memory for W\n");
        exit(1);
    }
    //!@#$%^&*
    a = posix_memalign((void **)&gradsq, 128, (words_size + contexts_size) * vector_size * sizeof(real)); // Might perform better than malloc
	if (gradsq == NULL) {
        fprintf(stderr, "Error allocating memory for gradsq\n");
        exit(1);
    }
    //!@#$%^&*
    for (b = 0; b < vector_size; b++) for (a = 0; a < words_size + contexts_size; a++) W[a * vector_size + b] = (rand() / (real)RAND_MAX - 0.5) / vector_size;
    for (b = 0; b < vector_size; b++) for (a = 0; a < words_size + contexts_size; a++) gradsq[a * vector_size + b] = 1.0; // So initial value of eta is equal to initial learning rate
    vector_size--;
}
...
void *glove_thread(void *vid) {
    ...
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        l2 = (cr.word2 + words_size) * (vector_size + 1); // shift by vocab_size to get separate vectors for context words
	...
	if (write_header) fprintf(fout, "%lld %d\n", words_size, vector_size); //!@#$%^&*
        for (a = 0; a < words_size; a++) { //!@#$%^&*
            ...
            if (model == 0) { // Save all parameters (including bias)
                ...
                for (b = 0; b < (vector_size + 1); b++) fprintf(fout," %lf", W[(words_size + a) * (vector_size + 1) + b]); //!@#$%^&*
            }
            ...
            if (model == 2) // Save "word + context word" vectors (without bias)
                for (b = 0; b < vector_size; b++) fprintf(fout," %lf", W[a * (vector_size + 1) + b] + W[(words_size + a) * (vector_size + 1) + b]); //!@#$%^&*
            fprintf(fout,"\n");
...
int main(int argc, char **argv) {
    ...
    //!@#$%^&*
    words_file = malloc(sizeof(char) * MAX_STRING_LENGTH);
    contexts_file = malloc(sizeof(char) * MAX_STRING_LENGTH);
	...
	if ((i = find_arg((char *)"-write-header", argc, argv)) > 0) write_header = atoi(argv[i + 1]);
        ...
        //!@#$%^&*
        if ((i = find_arg((char *)"-words-file", argc, argv)) > 0) strcpy(words_file, argv[i + 1]);
        else strcpy(words_file, (char *)"words.vocab");
        if ((i = find_arg((char *)"-contexts-file", argc, argv)) > 0) strcpy(contexts_file, argv[i + 1]);
        else strcpy(vocab_file, (char *)"contexts.vocab");
        ...
        while ((i = getc(fid)) != EOF) if (i == '\n') words_size++; // Count number of entries in vocab_file
        fclose(fid);

        contexts_size = 0;
        fid = fopen(contexts_file, "r");
        if (fid == NULL) {fprintf(stderr, "Unable to open contexts file %s.\n",contexts_file); return 1;}
        while ((i = getc(fid)) != EOF) if (i == '\n') contexts_size++; // Count number of entries in vocab_file
        fclose(fid);
