...
negative=5
corpus=wiki.en.txt
output_path=outputs/bi_bi

mkdir -p outputs/bi_bi/win${win}/sgns
python hyperwords/corpus2vocab.py --ngram 2 --reduce_thr 30000000 ${corpus} ${output_path}/win${win}/vocab
python hyperwords/corpus2pairs.py --win ${win} --sub ${sub} --ngram_word 2 --ngram_context 2 --thr ${thr} --threads ${threads} ${corpus} ${output_path}/win${win}/vocab ${output_path}/win${win}/pairs
